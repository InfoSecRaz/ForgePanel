...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-294420}" validate +quit
  exit 0
fi
...
exec ./7DaysToDieServer.x86_64 \
  -logfile /server/data/output.log \
  -quit -batchmode -nographics \
  -configfile=/server/data/serverconfig.xml \
  -dedicated
