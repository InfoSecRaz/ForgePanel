...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-565060}" validate +quit
  exit 0
fi
...
exec ./AvorionServer.sh -galaxyname "$GALAXY_NAME" -adminpassword "$ADMIN_PASSWORD" -port 27000
