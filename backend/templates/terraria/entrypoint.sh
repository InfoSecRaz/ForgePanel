...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-105600}" validate +quit
  exit 0
fi
...
exec ./TerrariaServer.bin.x86_64 -config serverconfig.txt -port 7777
