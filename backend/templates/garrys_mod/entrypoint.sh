...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-4020}" validate +quit
  exit 0
fi
...
exec ./srcds_run -game garrysmod -port 27015 +gamemode "$GAMEMODE" +map "${MAP:-gm_construct}" +maxplayers "$MAXPLAYERS" +sv_setsteamaccount "$GSLT" +rcon_password "$RCON_PASSWORD"
