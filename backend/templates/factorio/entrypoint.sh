...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-427520}" validate +quit
  exit 0
fi
...
exec ./bin/x64/factorio --start-server-load-latest --server-settings /server/data/server-settings.json --port 34197
