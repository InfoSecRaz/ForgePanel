...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-232250}" validate +quit
  exit 0
fi
...
exec ./srcds_run -game tf -port 27015 +map "${MAP:-cp_dustbowl}" +maxplayers "$MAXPLAYERS" +sv_setsteamaccount "$GSLT" +rcon_password "$RCON_PASSWORD"
