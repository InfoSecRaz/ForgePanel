...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-222860}" validate +quit
  exit 0
fi
...
exec ./srcds_run -game left4dead2 -port 27015 +map "${MAP:-c1m1_hotel}" +maxplayers "$MAXPLAYERS" +rcon_password "$RCON_PASSWORD"
