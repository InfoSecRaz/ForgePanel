...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-730}" validate +quit
  exit 0
fi
...
exec ./game/bin/linuxsteamrt64/cs2 -dedicated -port 27015 +map "${MAP:-de_dust2}" +sv_setsteamaccount "$GSLT" +rcon_password "$RCON_PASSWORD"
