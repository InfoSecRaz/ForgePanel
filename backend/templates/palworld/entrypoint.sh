...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-2394010}" validate +quit
  exit 0
fi
...
exec ./PalServer.sh -port=8211 -players="${MAXPLAYERS:-32}" -useperfthreads -NoAsyncLoadingThread -UseMultithreadForDS
