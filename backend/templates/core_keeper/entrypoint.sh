...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-1963720}" validate +quit
  exit 0
fi
...
exec ./CoreKeeperServer.x86_64 -batchmode -nographics
