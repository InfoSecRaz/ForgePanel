...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-1690800}" validate +quit
  exit 0
fi
...
exec ./FactoryServer.sh -unattended -Port=7777 -BeaconPort=15000
