...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-2857200}" validate +quit
  exit 0
fi
...
exec ./AbioticFactor/Binaries/Linux/AbioticFactorServer-Linux-Shipping
