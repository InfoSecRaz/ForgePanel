...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-2430930}" validate +quit
  exit 0
fi
...
exec ./ShooterGame/Binaries/Linux/ArkAscendedServer.sh \
  "${MAP:-TheIsland_WP}?listen?SessionName=${SERVER_NAME}?ServerPassword=${SERVER_PASSWORD}?ServerAdminPassword=${ADMIN_PASSWORD}?Port=7777?QueryPort=27015" \
  -server -log
