...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-2089300}" validate +quit
  exit 0
fi
...
exec ./Icarus/Binaries/Linux/IcarusServer-Linux-Shipping
