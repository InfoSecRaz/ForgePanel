...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +@sSteamCmdForcePlatformType windows +login "${STEAM_USER}" "${STEAM_PASS}" +app_update "${STEAMCMD_APPID:-2278520}" validate +quit
  exit 0
fi
