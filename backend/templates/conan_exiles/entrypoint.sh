...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-443030}" validate +quit
  exit 0
fi
...
exec ./ConanSandboxServer.sh -log -server-name "$SERVER_NAME"
