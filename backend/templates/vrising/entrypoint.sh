...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +login anonymous +app_update "${STEAMCMD_APPID:-1829350}" validate +quit
  exit 0
fi
...
exec ./VRisingServer.x86_64 -persistentDataPath /server/data/save-data -serverName "$NAME" -saveName world1
