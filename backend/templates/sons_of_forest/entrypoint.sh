...
if [ "$1" = "--install-only" ]; then
  run_steamcmd +force_install_dir /server/data +@sSteamCmdForcePlatformType windows +login anonymous +app_update "${STEAMCMD_APPID:-2465200}" validate +quit
  exit 0
fi
...
exec xvfb-run --auto-servernum wine64 SonsOfTheForestDS.exe
