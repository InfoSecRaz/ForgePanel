...
function parseUtc(sqliteTimestamp: string): Date | null {
...
export function formatUptime(server: Server): string {
  ...
  const ms = Date.now() - (parseUtc(server.updated_at)?.getTime() ?? 0);
...
export function formatDate(isoString?: string): string {
  if (!isoString) return '-';
  return (parseUtc(isoString) ?? new Date()).toLocaleDateString('en-US', {
...
export function formatRelativeTime(timestamp: string): string {
  const date = timestamp.includes('T') || timestamp.endsWith('Z')
    ? new Date(timestamp)
    : parseUtc(timestamp);
  if (!date) return '';
